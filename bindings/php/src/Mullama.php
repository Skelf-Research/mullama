...
final class Mullama
{
    ...
    private static function preprocessHeader(string $path): string
    {
        ...
        // Remove volatile typedef (FFI doesn't support volatile)
        $content = preg_replace('/typedef\s+struct\s*\{\s*volatile\s+int\s+value\s*;\s*\}\s+MullamaAtomicBool\s*;/', '', $content);

        // Add typedef without volatile
        $preamble = "typedef struct { int value; } MullamaAtomicBool;\n";
